...
( function( $ ) {
   ...
   jQuery('main p a').filter(function() {
   ...
   jQuery(".link-external").append('&nbsp;<span class="fa fa-caret-right" title="External Link"></span>');
   jQuery("main p a[href*=\\.csv]").after('&nbsp;<span class="fas fa-file-alt" title="Excel Document"></span>');
   jQuery("main p a[href*=\\.xlsx]").after('&nbsp;<span class="fas fa-file-excel" title="Excel Document"></span>');
   jQuery("main p a[href*=\\.pdf]").after('&nbsp;<span class="fas fa-file-pdf" title="PDF Document"></span>');
   jQuery("main p a[href*=\\.pptx]").after('&nbsp;<span class="fas fa-file-powerpoint" title="PowerPoint Document"></span>');
   jQuery("main p a[href*=\\.docx]").after('&nbsp;<span class="fas fa-file-word" title="Word Document"></span>');

   // Add icons to mailto links and buttons
   $.expr[':'].mailto = function(obj) {return obj.href.match(/^mailto\:/);};
   jQuery('main a:mailto').not(".list-icons a").after('&nbsp;<span class="fa fa-envelope" title="Email"></span>');
   jQuery('.btn-download').prepend('<span class="fas fa-download" title="Download"></span>&nbsp;');
   jQuery('.btn-go').append('&nbsp;<span class="fas fa-angle-right" title="Go To Link"></span>');
   jQuery('.btn-go-alt').append('&nbsp;<span class="fa fa-chevron-circle-right" title="Go To Link"></span>');

   // Add icons to .list-icons
   jQuery('.list-icons a').filter(function() {
      return host_t = this.hostname.substr(this.hostname.length - 7, 7), host_l = location.hostname.substr(location.hostname.length - 7, 7), host_t && host_t !== host_l;
   }).before('<span class="far fa-arrow-alt-circle-right"></span>&nbsp;');
   jQuery(".list-icons a[href*=\\.csv]").before('<span class="fas fa-file-alt" title="Comma Separated File"></span>&nbsp;');
   jQuery(".list-icons a[href*=\\.xlsx]").before('<span class="fas fa-file-excel" title="Excel Document"></span>&nbsp;');
   jQuery(".list-icons a[href*=\\.pdf]").before('<span class="fas fa-file-pdf" title="PDF Document"></span>&nbsp;');
   jQuery(".list-icons a[href*=\\.pptx]").before('<span class="fas fa-file-powerpoint" title="PowerPoint Document"></span>&nbsp;');
   jQuery(".list-icons a[href*=\\.docx]").before('<span class="fas fa-file-word" title="Word Document"></span>&nbsp;');
   $.expr[':'].fax = function(obj) {return obj.href.match(/^fax\:/);};
   jQuery('ul.list-icons a:fax').before('<span class="fa fa-fax" title="Fax"></span>&nbsp;');
   $.expr[':'].mailto = function(obj) {return obj.href.match(/^mailto\:/);};
   jQuery('ul.list-icons a:mailto').before('<span class="fa fa-envelope" title="Email"></span>&nbsp;');
   $.expr[':'].tel = function(obj) {return obj.href.match(/^tel\:/);};
   jQuery('ul.list-icons a:tel').before('<span class="fa fa-phone" title="Phone"></span>&nbsp;');
   jQuery('ul.list-icons li.user a').before('<span class="fas fa-user" title="User"></span>&nbsp;');
   jQuery('ul.list-icons li.map a').before('<span class="fa fa-map-marker-alt" title="Location"></span>&nbsp;');

   // Menu arrow toggle
   jQuery('.btn-menu').click(function(){
      jQuery(this).find("span").toggleClass("fa-chevron-circle-down fa-chevron-circle-up");
    });

    // Add the .s3-gt class to buttons with .btn-primary
    jQuery('.btn-primary').addClass('s3-gt');

    // Add the .null class to images within links (removes background-image)
    jQuery('a').has('img').addClass('null');
